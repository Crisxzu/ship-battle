...
import com.badlogic.gdx.utils.Value;
...
public class SetupPlayerShipView extends GuiView<SetupMenuController> {
    ...
    protected void buildUI() {
        ...
        root.pad(Value.percentWidth(0.02f, root));
        stage.addActor(root);

        Table leftColumn = new Table();
        leftColumn.defaults().pad(Value.percentHeight(0.01f, root)).growX();
        ...
        for (int y = height - 1; y >= 0; y--) {
            for (int x = 0; x < width; x++) {
                ...
                gridTable.add(cellActor)
                    .size(Value.percentWidth(0.03f, root), Value.percentWidth(0.03f, root));
            }
            gridTable.row();
        }
    }
